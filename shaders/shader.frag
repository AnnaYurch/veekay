...
layout (set = 0, binding = 2, std430) readonly buffer SpotLightsBuffer {
...
float calculateShadow(vec4 lightSpacePos, vec3 normal, vec3 lightDir, sampler2DShadow shadowSampler) {
    ...
    //помогает бороться с теневой рябью
    float bias = max(0.005 * (1.0 - dot(normal, lightDir)), 0.0005);

    //усредняет 4 соседних пикселя и выдает освещено или нет
    float shadow = texture(shadowSampler, vec3(projCoords.xy, projCoords.z - bias));

    return shadow; //1.0 свет 
}
...
vec3 calculateBlinnPhong(vec3 lightDir, vec3 lightColor, vec3 normal, vec3 viewDir, vec3 albedo) {
    ...
    float spec = pow(max(dot(normal, halfDir), 0.0), model_ubo.shininess);
...
void main() {
    ...
    vec3 albedoWithTexture = model_ubo.albedo_color * texColor.rgb;
